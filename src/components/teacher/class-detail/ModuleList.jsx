...
export default function ModuleList({ modules, departmentId, onRemove, onUpdate }) {
  ...
  const handleRemove = async (moduleId) => {
    ...
    try { await onRemove?.(moduleId) } finally { setRemoving(null); setConfirmId(null) }
  }
  ...
  return (
    ...
    <div className="cd-list">
      <div className="cd-list-header">
        ...
        <span className="cd-col cd-col--module">Module</span>
      ...
      {modules.length === 0 ? (
        <p className="cd-empty">No modules match your search.</p>
      ) : (
        modules.map((l, i) => {
          ...
          return (
            <div
              key={l.id}
              className="cd-row cd-row--module"
              style={{ animationDelay: `${Math.min(i, 6) * 0.04}s` }}
            >
              <div className="cd-col cd-col--num">
                <span className="cd-module-num">{l.num}</span>
              </div>
              <div className="cd-col cd-col--module">
                <span className="cd-module-title">{l.title}</span>
              </div>
                ...
                {isConf ? (
                  ...
                  <div className="cd-row-actions">
                    <button
                      ...
                      title="Edit panels"
                      onClick={() => navigate(`/teacher/modules/${l.id}/panels`)}
                    >
    ...
    {editLesson && (
      <ModuleEditModal
        module={{ id: editLesson.id, title: editLesson.title, duration_minutes: parseInt(editLesson.duration) || '' }}
        onSave={handleSave}
