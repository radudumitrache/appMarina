...
function CourseProgressView({ data, loading, diplomas, onAwardStudent }) {
  ...
  if (lessons.length === 0) {
    return <p className="lct-cl-empty">This course has no lessons yet.</p>
  }

  const useDots = lessons.length <= 12
...
export default function LessonsCoursesTab({ departmentId, classLessons, onNewLesson, onClassLessonUpdate }) {
  ...
  // per-course view: 'lessons' | 'progress'
  ...
  function getCourseView(id) { return courseViews[id] || 'lessons' }
              ...
              {isExpanded && (
                <>
                  ...
                  <div className="lct-inner-tab-bar">
                    <button
                      className={`lct-inner-tab${view === 'lessons' ? ' lct-inner-tab--active' : ''}`}
                      onClick={() => setCourseViews(prev => ({ ...prev, [course.id]: 'lessons' }))}
                    >
                      Lessons
                    </button>
                    <button
