...
const tileVariants = {
  ...
  show: (i) => ({
    opacity: 1,
    y: 0,
    transition: { delay: Math.min(i, 6) * 0.04 + 0.52, duration: 0.38, ease: SPRING },
  }),
}

const NAV_TILES = [
  {
    id: 'modules', label: 'Courses', path: '/crew/modules',
    icon: (
  ...
  {
    id: 'progress', label: 'My Progress', path: '/crew/progress',
    icon: (
      <svg width="26" height="26" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
  ...
  {
    id: 'class', label: 'My Department', path: '/crew/my-class',
    icon: (
      <svg width="26" height="26" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
...
export default function DashNavGrid({ uiLeaving, username }) {
    ...
    >
      ...
      >
        ...
        <p className="dash-greeting">
          Welcome back,{' '}
          <span className="dash-username">{username ?? 'Crew Member'}</span>
        </p>
      </motion.div>

      <nav className="dash-grid" aria-label="Crew Member navigation">
        {[NAV_TILES.slice(0, 2), NAV_TILES.slice(2)].map((row, rowIdx) => (
          <div className="dash-row" key={rowIdx}>
            {row.map((tile, i) => (
              <motion.button
                key={tile.id}
                className="nav-tile"
