import '../../css/student/modules/ModulesSidebar.css'
import { CATEGORIES } from '../../../pages/student/Modules'

function getCatStats(modules, catId) {
  const subset = catId === 'all' ? modules : modules.filter(l => l.cat === catId)
  return { total: subset.length, done: subset.filter(l => l.complete).length }
}

export default function ModulesSidebar({ modules, activeCategory, onCategoryChange, className = '' }) {
  const overall    = getCatStats(modules, 'all')
  ...
  return (
    <aside className={`sidebar${className ? ` ${className}` : ''}`}>
      <nav className="sidebar-nav">
        {CATEGORIES.map((cat) => {
          const stats = getCatStats(modules, cat.id)
