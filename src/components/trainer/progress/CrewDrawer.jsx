...
import '../../css/trainer/progress/CrewDrawer.css'
...
export default function CrewDrawer({ crew, onClose }) {
  const [tests,        setTests]        = useState([])
  const [loadingTests, setLoadingTests] = useState(true)

  useEffect(() => {
    if (!crew.departmentId) { setLoadingTests(false); return }
    setLoadingTests(true)
    getClassAssignments(crew.departmentId)
      .then(async ({ data: assignments }) => {
        const withScores = await Promise.all(
          assignments.map(async (test) => {
            try {
              const { data: subs } = await getAllSubmissions(test.id)
              const sub = subs.find(s => s.crew === crew.id)
  ...
  }, [crew.id, crew.departmentId])

  const pct = crew.modulesTotal > 0
    ? Math.round((crew.modulesDone / crew.modulesTotal) * 100)
    : 0
    ...
    <>
      ...
      <aside className="sd-drawer">
        <div className="sd-header">
          ...
          <div className="sd-id">
            <span className="sd-name">{crew.name}</span>
            <span className="sd-class-name">{crew.className}</span>
          </div>
        ...
        <div className="sd-body">
          ...
          <section className="sd-section">
            ...
            <div className="sd-module-count">
              {crew.modulesDone} of {crew.modulesTotal} modules completed
            </div>
