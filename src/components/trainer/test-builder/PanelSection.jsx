...
import { Q_TYPES } from '../../../pages/trainer/testBuilderMock'
import '../../css/trainer/test-builder/QuestionCard.css'
import '../../css/trainer/test-builder/AddQuestionPanel.css'
import '../../css/trainer/test-builder/PanelSection.css'
        ...
        <div className="tb-anchor-points-list">
          {points.map((pt, pi) => (
            <div key={pt.id} className="tb-anchor-point-row">
              ...
              <button type="button" className="tb-opt-del" onClick={() => handleDeletePoint(pt.id)}>&times;</button>
            </div>
          ))}
        </div>
...
function VRPanelSection({ vr, panelId, testId, departmentId, panelTitle, vrBasePath, onReload, canWrite }) {
  ...
  function openVREditor() {
    const base = vrBasePath ?? '/trainer/assignments'
...
export default function PanelSection({ panel, testId, departmentId, vrBasePath, index, expanded, onExpand, onReload }) {
  const { user } = useAuth()
  const canWrite = user?.role === 'trainer' || user?.role === 'admin' || !!user?.is_staff
