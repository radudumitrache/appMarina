...
import '../../css/trainer/class-detail/AssignModuleModal.css'

export default function AssignModuleModal({ departmentId, assignedIds = [], onClose, onAssigned }) {
  ...
  const filtered = modules.filter(l => {
  ...
  const handleAssign = async () => {
    ...
    setError(null)
    try {
      const { data } = await assignModule(departmentId, { module: selected })
      onAssigned(data)
      onClose()
    } catch {
    ...
    <div className="alm-backdrop" onClick={onClose}>
      <div className="alm-modal" onClick={e => e.stopPropagation()}>
        <div className="alm-header">
          <h3 className="alm-title">Assign Module</h3>
        ...
        <div className="alm-list">
          {loading ? (
            <p className="alm-empty">Loading modulesâ€¦</p>
          ) : filtered.length === 0 ? (
            <p className="alm-empty">No modules found.</p>
          ) : (
            filtered.map(l => {
        ...
        <div className="alm-footer">
          ...
          <button className="alm-btn-primary" onClick={handleAssign} disabled={!selected || saving}>
            {saving ? 'Assigningâ€¦' : 'Assign Module'}
          </button>
        </div>
      </div>
    </div>
