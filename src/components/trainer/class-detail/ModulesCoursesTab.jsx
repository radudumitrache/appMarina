...
import '../../css/trainer/class-detail/ModulesCoursesTab.css'
...
export default function ModulesCoursesTab({ departmentId, classModules, onNewModule, onModuleUpdate }) {
  ...
  const handleRemoveFromCourse = async ({ courseId, moduleId }) => {
    ...
    catch { getCourse(courseId).then(d => setCourseModules(prev => ({ ...prev, [courseId]: (d.data.modules ?? []).map(cl => ({ id: cl.id, moduleId: cl.module ?? null, testId: cl.test ?? null, title: cl.module_detail?.title ?? cl.test_detail?.title ?? '(untitled)', itemType: cl.test_detail ? 'test' : 'module' })) }))).catch(() => {}) }
  }
                        ...
                        cls.map((l, i) => {
                          ...
                          return (
                            <div key={l.id} className="lct-cl-item">
                              ...
                              {l.itemType === 'module' && l.moduleId ? (
                                <button className="lct-cl-title lct-cl-title--link" onClick={() => navigate(`/trainer/modules/${l.moduleId}`)}>
                                  {l.title}
                                </button>
                              ) : l.itemType === 'test' && l.testId ? (
                                <button className="lct-cl-title lct-cl-title--link" onClick={() => navigate(`/trainer/assignments?test=${l.testId}`)}>
                                  {l.title}
                                </button>
                              ) : (
