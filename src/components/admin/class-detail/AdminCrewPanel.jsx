...
import '../../css/admin/class-detail/AdminCrewPanel.css'

function DiplomaDropdown({ crewMember, diplomas, onAward, onRevoke, onClose }) {
  ...
  async function toggle(diploma) {
    ...
    setBusy(true)
    try {
      if (awarded) await onRevoke(diploma.id, crewMember.id)
      else await onAward(diploma.id, crewMember.id)
    } finally { setBusy(false) }
  }

  return (
    <div className="asp-dip-drop" ref={ref}>
      <div className="asp-dip-drop-hd">Diplomas — {crewMember.name.split(' ')[0]}</div>
      {diplomas.length === 0 ? (
...
export default function AdminCrewPanel({
  crew,
  diplomas = [],
  ...
  return (
    <div className="asp-panel">
      <div className="asp-header">
        <span className="asp-title">Crew Members</span>
        <span className="asp-count">{crew.length}</span>
      </div>
      ...
      <div className="asp-add-wrap">
        <div className="asp-search-box">
          ...
          <input
            ...
            placeholder="Enroll a crew member..."
            value={searchValue}
      ...
      {crew.length > 0 && (
        <div className="asp-list-hd">
          <span className="asp-col asp-col--crew">Crew Member</span>
      ...
      <div className="asp-list">
        {crew.length === 0 ? (
          <p className="asp-empty">No crew members enrolled yet.</p>
        ) : (
          crew.map((s, i) => {
            const p      = pct(s)
            ...
            return (
              ...
              >
                ...
                <div className="asp-info">
                  <button className="asp-name" onClick={() => navigate(`/admin/crew/${s.id}/progress`)}>
                    {s.name}
                  </button>
                ...
                <div className="asp-actions">
                  ...
                  <button className="asp-act-btn asp-act-btn--remove" onClick={() => onRemove(s.id)} title="Remove crew member">
