...
import { getTeacherSubmission, patchTeacherSubmission } from '../../api/teacher'
import Sk from '../../components/shared/Skeleton'
import '../css/teacher/SubmissionOverview.css'

export default function TeacherSubmissionOverview() {
  ...
  const handleToggle = useCallback(async (answerId, newIsCorrect) => {
    setSaving(true)
    try { applyResponse(await patchTeacherSubmission(submissionId, { answers: [{ answer_id: answerId, is_correct: newIsCorrect }] })) }
    catch {}
    setSaving(false)
  }, [submissionId])
  ...
  const handleGradeSave = async () => {
    ...
    setGradeSaving(true)
    try { applyResponse(await patchTeacherSubmission(submissionId, { grade: parsed })) }
    catch {}
  ...
  if (loading) {
    return (
      <div className="so-page so-page--teacher">
        <NavBar />
