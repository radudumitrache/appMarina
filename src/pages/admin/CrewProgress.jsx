...
import { getCrewProgress } from '../../api/admin'
import { getDiplomas, awardDiploma, revokeDiploma } from '../../api/departments'
import { gradeColor, gradeLabel, fmt } from '../../components/admin/crew-progress/helpers'
import Sk from '../../components/shared/Skeleton'
import '../css/admin/CrewProgress.css'

// ── Diploma dropdown ────────────────────────────────────────────────────────

function DiplomaDropdown({ crewId, diplomas, onAward, onRevoke, onClose }) {
  ...
  async function toggle(d) {
    const awarded = (d.recipients ?? []).some(r => r.id === crewId)
    setBusy(true)
    try {
      if (awarded) await onRevoke(d.department_id, d.id, crewId)
      else         await onAward(d.department_id, d.id, crewId)
  ...
  return (
    <div className="sp-dip-drop" ref={ref}>
      ...
      {diplomas.length === 0
        ? <p className="sp-dip-empty">No diplomas found for this crew member's departments.</p>
        : diplomas.map(d => {
            const awarded = (d.recipients ?? []).some(r => r.id === crewId)
            return (
...
function CourseCard({ course, index, crewId, diplomas, onAward, onRevoke }) {
...
export default function AdminCrewProgress() {
  ...
  useEffect(() => {
    getCrewProgress(crewId)
      ...
      .catch(() => setError('Could not load crew member progress.'))
      .finally(() => setLoading(false))
  }, [crewId])

  const handleAward = async (deptId, dipId, cid) => {
    const { data: updated } = await awardDiploma(deptId, dipId, { crew_ids: [cid] })
    setDiplomas(prev => prev.map(d => d.id === updated.id && d.department_id === deptId ? { ...updated, department_id: deptId } : d))
  }

  const handleRevoke = async (deptId, dipId, cid) => {
    await revokeDiploma(deptId, dipId, cid)
    setDiplomas(prev => prev.map(d =>
      d.id === dipId && d.department_id === deptId
        ? { ...d, recipients: (d.recipients ?? []).filter(r => r.id !== cid) }
        : d
    ))
  }
  ...
  const initials = (data.crew_name || '?').split(' ').map(w => w[0]).join('').slice(0, 2).toUpperCase()
      ...
      <div className="sp-shell">
        ...
        <div className="sp-hd">
          ...
          <div className="sp-hd-info">
            <h1 className="sp-hd-name">{data.crew_name}</h1>
        ...
        <div className="sp-courses-section">
          ...
          {courses.length === 0
            ? <p className="sp-empty">No courses assigned to this crew member's departments.</p>
