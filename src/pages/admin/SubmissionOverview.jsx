...
import { getSubmission, patchSubmission } from '../../api/admin'
import Sk from '../../components/shared/Skeleton'
import '../css/admin/SubmissionOverview.css'
...
export default function AdminSubmissionOverview() {
  ...
  const handleToggle = useCallback(async (answerId, newIsCorrect) => {
    setSaving(true)
    try { applyResponse(await patchSubmission(submissionId, { answers: [{ answer_id: answerId, is_correct: newIsCorrect }] })) }
    catch {}
    setSaving(false)
  }, [submissionId])
  ...
  const handleGradeSave = async () => {
    ...
    setGradeSaving(true)
    try { applyResponse(await patchSubmission(submissionId, { grade: parsed })) }
    catch {}
